        ...
        for y, row in enumerate(self.board):
            for x, part in enumerate(row):
                if part:
                    ...
                    pos = f'{str(self.columns[x]).upper() if p_colour > 0 else str(self.columns[x]).lower()}{self.rows[y]}'
                    moves[pos] = v_moves
...
# After running MCTS, select the move with the highest visit count as the best move
